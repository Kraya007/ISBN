...
public class TypingTutorApp {
	...
	static HungryWord h_word = new HungryWord(); // The hungry word
	static HungryWordMover h_side; // hungry word mover
	...
	public static void setupGUI(int frameX, int frameY, int yLimit) {
		...
		gameWindow = new GamePanel(words, h_word, yLimit, done, started, won);
		...
		textEntry.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				if (!pause.get()) {
					...
					FallingWord l_word = null;
					int index = -1; // indes of the lowest duplicate
					for (int i = 0; i < noWords; i++) {
						if (text.equals(words[i].getWord())) {
							if (l_word == null || l_word.getY() < words[i].getY()) {
								l_word = words[i];
								index = i;
							}
						}
	...
	public static void createWordMoverThreads() {
		...
		h_word.setWord(dict.getNewWord());
		h_word.setY(frameY / 2 - 50);
		h_word.setMaxX(gameWindow.getWidth() - 50);

		h_side = new HungryWordMover(h_word, dict, score, startLatch, done, pause);
		h_side.setWords(words);
		h_side.start();
		// word movers waiting on starting line
		for (int i = 0; i < noWords; i++) {
			wrdShft[i].start();
	...
	public static void main(String[] args) {
		...
		CatchWord.setHungryWord(h_word);
